...
ClientGameHandler::ClientGameHandler(bool debugMode)
  ...
  _renderer.loadSpriteSheet("textures/settingmenu/Keybinds.png", "keybinds", 16,
                            16);
...
int ClientGameHandler::run() {
  ...
  while (running) {
    ...
    _spritesys.render(_reg, [&](const SpriteSystem::TextureId &tid, int width,
    ...
    _spritesheetsys.render(
        _reg,
        [&](const SpriteSheetSystem::TextureId &tid, int frameIndex, int width,
            int height, int x, int y, int z) {
    ...
    if (_debugMode) {
      ...
      for (auto const &[serverEntity, localEntity] : playerEntities) {
        if (_reg.hasComponent<Position>(localEntity)) {
          ...
          Color boxColor = (localEntity == myEntity) ? Color{0, 255, 0, 255}
                                                     : Color{0, 255, 255, 255};
      ...
      for (auto const &[serverEntity, localEntity] : projectileEntities) {
        if (_reg.hasComponent<Position>(localEntity)) {
          ...
          _renderer.drawRect(Rect{(int)pos.x, (int)pos.y, 16, 16},
                             Color{255, 255, 0, 255}, RenderLayer::OVERLAY,
                             100);
        }
      }
    }
    ...
    _labelsys.render(_reg, [&](const LabelSystem::TextId &tid,
                               std::string &text, int x, int y, Color color) {
      _renderer.drawFontAndCache(tid, text, x, y, color, RenderLayer::OVERLAY,
                                 0);
    });
        ...
        if (sp.textureIndex == "obelisk_void") {
          ...
          if (posArr->has(e)) {
            ...
            _renderer.drawCircle((int)p.x + 50, (int)p.y + 50, 50,
                                 Color(255, 0, 0, alpha), RenderLayer::GAME, 10,
                                 true);
            _renderer.drawCircle((int)p.x + 50, (int)p.y + 50, 50,
                                 Color(255, 0, 0, 255), RenderLayer::GAME, 11,
                                 false);
    ...
    if (_reg.hasComponent<Stats>(myEntity)) {
      ...
      if (stats.maxHp > 0) {
        float healthPct =
            static_cast<float>(stats.hp) / static_cast<float>(stats.maxHp);
        if (healthPct < 0.0f)
          healthPct = 0.0f;
        if (healthPct > 1.0f)
          healthPct = 1.0f;
...
void ClientGameHandler::cleanupServerEntity(Entity serverEntity) {
  ...
  if (itComp != companionEntities.end()) {
    std::cout << "Warning: Cleaning up old companion entity with serverEntity "
              << serverEntity << std::endl;
  ...
  while (_network.hasMessages()) {
    ...
    case OpCode::CONNECT_ACK: {
      if (msg->data.size() >= 4) {
        ...
        if (getsockname(udpFd, (struct sockaddr *)&localAddr, &addrLen) == 0) {
          ...
          std::cout << "Sent LINK message via UDP, listening on port: "
                    << localPort << std::endl;
        } else {
        ...
        if (!_joinedRoom) {
          ...
          PreparedMessage createMsg =
              factory.createMessage(OpCode::CREATE_ROOM, {});
    ...
    case OpCode::ROOM_CREATED: {
      if (msg->data.size() >= 4) {
        ...
        std::cout << "Room created with ID: " << roomId << ". Joining..."
                  << std::endl;
        ...
        PreparedMessage joinMsg =
            factory.createMessage(OpCode::JOIN_ROOM, payload);
      ...
      if (msg->data.size() >= 26) {
        ...
        if (itOldEnemy != enemyEntities.end()) {
          std::cout << "[INFO] Cleaning up stale ENEMY (ID: "
                    << serverProjectileEntity << ") for new PROJECTILE"
                    << std::endl;
        ...
        if (ownerType == "player") {
          ...
          if (it != playerEntities.end()) {
            ...
            _reg.addComponent<SpriteSheets>(
                projectile, std::string(""), std::string("projectile_player"),
                size, size, 0, 4, 0, 0.f, 0.f, true, true);
          ...
          std::cout << "Invalid ownerType in SHOOT message: " << ownerType
                    << std::endl;
        }
      }
      break;
      ...
      if (msg->data.size() >= 28) {
        ...
        if (it == enemyEntities.end()) {
          ...
          if (companionEntities.find(serverEntity) != companionEntities.end()) {
            std::cout
                << "[INFO] Ignoring stale ENEMY packet for existing COMPANION "
                << serverEntity << std::endl;
            break;
          }
          if (playerEntities.find(serverEntity) != playerEntities.end()) {
            std::cout
                << "[INFO] Ignoring stale ENEMY packet for existing PLAYER "
                << serverEntity << std::endl;
            break;
          }
          ...
          for (const auto &[id, config] : bosses) {
            if (config.name == typeStr) {
              ...
              _reg.addComponent<SpriteSheets>(
                  localEntity, config.visuals.texture_path, config.name, w, h,
                  0, frames, 4, config.visuals.offset_x,
          ...
          if (!loadedFromConfig) {
            if (typeStr == "Ancient Obelisk") {
              _reg.addComponent<SpriteSheets>(
                  ...
                  std::string("Ancient Obelisk"), 240, 160, 0, 14, 4, 0.f, 0.f,
                  true, true);
              std::cout << "Created BOSS entity (serverId: " << serverEntity
    ...
    case OpCode::COMPANION: {
      if (msg->data.size() >= 13) {
        ...
        std::cout << "[DEBUG] COMPANION OpCode Received: ID=" << serverEntity
                  << " X=" << x << " Y=" << y << std::endl;
      ...
      if (msg->data.size() >= 5) {
        ...
        case EntityType::PROJECTILE: {
          ...
          if (itProjectile != projectileEntities.end()) {
            ...
            std::cout << "Projectile " << serverEntity << " destroyed"
                      << std::endl;
          }
          break;
        }
        ...
        case EntityType::COMPANION: {
          ...
          if (it != companionEntities.end()) {
            ...
            std::cout << "Companion " << serverEntity << " destroyed"
                      << std::endl;
          }
          break;
        }
...
void ClientGameHandler::handlePlayerPacket(const DecodedMessage &msg) {
  ...
  if (it == playerEntities.end()) {
    ...
    _reg.addComponent<SpriteSheets>(localEntity, std::string(""), selectedSkin,
                                    sprW, sprH, 1, 3, 0, 0.f, 0.f, true, false);
    _reg.addComponent<Stats>(localEntity, pStats.health, pStats.max_health,
                             pStats.attack_speed, 0.f, pStats.attack_damage, 1,
                             pStats.speed);
...
void ClientGameHandler::handleUpdateWeapon(const DecodedMessage &msg) {
  ...
  if (itPlayer != playerEntities.end()) {
    ...
    if (_reg.hasComponent<Weapon>(localEntity)) {
      ...
      w.fireRate = fireRate;
      std::cout << "Updated Weapon for player " << serverEntity << " (local "
                << localEntity << "): FR=" << fireRate << " DMG=" << damage
                << std::endl;
    } else {
      _reg.addComponent<Weapon>(localEntity, damage, nbBullets, fireRate);
      std::cout << "Added Weapon for player " << serverEntity << " (local "
                << localEntity << ")" << std::endl;
    }
  }
}
