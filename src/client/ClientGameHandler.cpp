...
ClientGameHandler::ClientGameHandler(bool debugMode)
    ...
    _renderer.loadFont("font/josefin-sans/JosefinSans-Regular.ttf", 25,
                       "default_font_small");
    _renderer.loadFont("font/josefin-sans/JosefinSans-Regular.ttf", 14,
                       "default_font_medium");
    _renderer.loadFont("font/josefin-sans/JosefinSans-Regular.ttf", 15,
                       "default_font_tiny");
    _renderer.loadTexture("textures/ships/enemy_ship.png", "enemy_ship");
    // Dynamic Boss Loading
    const auto &bosses = _config.getBosses();
    if (bosses.empty()) {
        std::cout << "[WARNING] No boss configurations loaded for client."
                  << std::endl;
        // Fallback just in case
        _renderer.loadSpriteSheet("textures/ships/bosses/Obelisk.png",
                                  "Ancient Obelisk", 240, 160);
    } else {
        for (const auto &[id, config] : bosses) {
            ...
            std::cout << "[INFO] Loading Boss Sprite: " << config.name << " ("
                      << texturePath << ") " << w << "x" << h << std::endl;
            // Ensure texture path is relative to what renderer expects if
            // needed
    ...
    _reg.addComponent<Sprite>(
        start_button, (std::string) "textures/play_button/default.png",
...
int ClientGameHandler::run() {
    ...
    while (running) {
        ...
        for (auto it = projectileEntities.begin();
             it != projectileEntities.end();) {
            ...
            if (_reg.hasComponent<Position>(localEntity)) {
                ...
                if (pos.x < -100 || pos.x > 1920 || pos.y < -100 ||
                    pos.y > 1080) {
                    if (_debugMode) {
                        std::cout
                            << "Cleaning up out-of-bounds projectile (localId: "
            ...
            for (auto const &[serverEntity, localEntity] : enemyEntities) {
                if (_reg.hasComponent<Position>(localEntity)) {
                    ...
                    if (_reg.hasComponent<Sprite>(localEntity)) {
                        ...
                        // Enemies usually have 50x50 or 60x60 depending on
                        // type, but for simple visualization using sprite size
                        // + red box
                        ...
                        // float hx = pos.x + sprite.offset_x; // Wait, sprite
                        // offset is for VISUALS. Hitbox is usually separate?
                        // But here we draw "Hitbox based on sprite".
                        // For Boss, Hitbox is config based, but we don't have
                        // easy access to that config per-entity here without
                        // lookup. Let's iterate bosses to find match? Too slow.
        ...
        if (_reg.hasComponent<Stats>(myEntity)) {
            ...
            if (stats.maxHp > 0) {
                float healthPct = static_cast<float>(stats.hp) /
...
void ClientGameHandler::cleanupServerEntity(Entity serverEntity) {
    ...
    if (itProj != projectileEntities.end()) {
        // Warning log might be noisy for projectiles but useful for debugging
        // this issue
        std::cout << "Cleaning up old projectile entity with serverEntity "
                  << serverEntity << std::endl;
    ...
    if (itComp != companionEntities.end()) {
        std::cout
            << "Warning: Cleaning up old companion entity with serverEntity "
            << serverEntity << std::endl;
    ...
    while (_network.hasMessages()) {
        ...
        case OpCode::CONNECT_ACK: {
            if (msg->data.size() >= 4) {
                ...
                PreparedMessage linkMsg =
                    factory.createMessage(OpCode::LINK, linkPayload);
                _network.sendUdp(linkMsg);

                struct sockaddr_in localAddr;
                socklen_t addrLen = sizeof(localAddr);
                int udpFd = _network.getUdpClient().getSocketFd();
                if (getsockname(udpFd, (struct sockaddr *)&localAddr,
                                &addrLen) == 0) {
                    uint16_t localPort = ntohs(localAddr.sin_port);
                    std::cout
                        << "Sent LINK message via UDP, listening on port: "
                        << localPort << std::endl;
                } else {
        ...
        case OpCode::SHOOT: {
            ...
            if (msg->data.size() >= 26) {
                ...
                // Companions/Players if ID reuse happens rapidly or
                // out-of-order.
                auto itOldProj =
                    projectileEntities.find(serverProjectileEntity);
                ...
                if (itOldEnemy != enemyEntities.end()) {
                    std::cout << "[INFO] Cleaning up stale ENEMY (ID: "
                ...
                if (ownerType == "player") {
                    ...
                    if (it != playerEntities.end()) {
                        ...
                        std::cout << "Received SHOOT from unknown entity: "
                                  << serverParentEntity << std::endl;
                    }
                    ...
                    // Use a simple sprite for boss orb, or fallback to player
                    // projectile if texture missing We try to load a
                    // distinctive texture
                    _renderer.loadTexture("textures/projectiles/boss_orb.png",
                                          "boss_orb");
                    // If file missing, renderer usually logs error and returns
                    // empty or fallback. But let's assume if it fails we can't
                    // easily check without helper. We will use Sprite, assuming
                    // boss_orb.png exists or we use enemy_ship as fallback
                    // visual? Let's rely on standard projectile_player sprite
                    // sheet but maybe frame index? No, let's just make it a
                    // Sprite.
                    _reg.addComponent<Sprite>(
                        projectile,
                        std::string("textures/projectiles/boss_orb.png"),
                        std::string("boss_orb"), size, size, 10, 0.f, 0.f,
                        true);
        ...
        case OpCode::ENEMY: {
            if (msg->data.size() >= 28) {
                ...
                if (it == enemyEntities.end()) {
                    // Check if this ID is already used by another type (stale
                    // packet check)
                    if (companionEntities.find(serverEntity) !=
                        companionEntities.end()) {
                        std::cout << "[INFO] Ignoring stale ENEMY packet for "
                                     "existing COMPANION "
                                  << serverEntity << std::endl;
                        break;
                    }
                    if (playerEntities.find(serverEntity) !=
                        playerEntities.end()) {
                        std::cout << "[INFO] Ignoring stale ENEMY packet for "
                                     "existing PLAYER "
                                  << serverEntity << std::endl;
                    ...
                    for (const auto &[id, config] : bosses) {
                        if (config.name == typeStr) {
                            ...
                            if (config.visuals.animations.count("idle")) {
                                const auto &anim =
                                    config.visuals.animations.at("idle");
                                w = anim.width;
                                h = anim.height;
                            ...
                            _reg.addComponent<SpriteSheets>(
                                localEntity, config.visuals.texture_path,
                                config.name, w, h, 0, frames, 4,
                                config.visuals.offset_x,
                                config.visuals.offset_y, true, true);
                            std::cout << "Created BOSS entity (serverId: "
                                      << serverEntity
                                      << ") via Config: " << config.name
                    ...
                    if (!loadedFromConfig) {
                        if (typeStr == "Ancient Obelisk") {
                            _reg.addComponent<SpriteSheets>(
                                localEntity,
                                std::string(
                                    "textures/ships/bosses/Obelisk.png"),
                                std::string("Ancient Obelisk"), 240, 160, 0, 14,
                                4, 0.f, 0.f, true, true);
                            std::cout << "Created BOSS entity (serverId: "
                                      << serverEntity << ") type: " << typeStr
                                      << " (Fallback)" << std::endl;
                        } else if (typeStr == "void_zone") {
                            _renderer.loadTexture(
                                "textures/ships/bosses/Obelisk_effects.png",
                                "obelisk_void");
                            _reg.addComponent<Sprite>(
                                localEntity,
                                std::string("textures/ships/bosses/"
                                            "Obelisk_effects.png"),
                                std::string("obelisk_void"), 100, 100, 5, -50.f,
                                -50.f, true);
                            std::cout << "Created VOID ZONE entity (serverId: "
                                      << serverEntity << ")" << std::endl;
                        } else {
                            _reg.addComponent<Sprite>(
                                localEntity,
                                std::string("textures/ships/enemy_ship.png"),
                                std::string("enemy_ship"), 50, 50, 0, 0.f, 0.f,
                                true);
                            std::cout << "Created enemy entity (serverId: "
                                      << serverEntity << ") type: " << typeStr
                                      << std::endl;
                        }
                    }
        ...
        case OpCode::COMPANION: {
            if (msg->data.size() >= 13) {
                ...
                std::cout << "[DEBUG] COMPANION OpCode Received: ID="
                          << serverEntity << " X=" << x << " Y=" << y
                          << std::endl;
                ...
                if (it == companionEntities.end()) {
                    ...
                    // Use player_ship texture for drone (since vaisseau.png is
                    // missing) We scale it down to 30x30 to look like a small
                    // drone
                    _reg.addComponent<Sprite>(
                        localEntity,
                        std::string("textures/ships/player_ship.png"),
                        std::string("drone"), 30, 30, 15, 0.f, 0.f, true);
                    std::cout << "Created COMPANION entity (serverId: "
                              << serverEntity << ")" << std::endl;
                ...
                std::cerr << "[ERROR] COMPANION packet size too small: "
                          << msg->data.size() << std::endl;
            }
            break;
        }
            ...
            if (msg->data.size() >= 16) {
                ...
                if (it != playerEntities.end()) {
                    ...
                    if (_reg.hasComponent<Stats>(localEntity)) {
                        ...
                        std::cout << "Updated Stats for player " << serverEntity
                                  << ": HP=" << hp << "/" << maxHp
            ...
            if (msg->data.size() >= 5) {
                ...
                case EntityType::COMPANION: {
                    ...
                    if (it != companionEntities.end()) {
                        ...
                        std::cout << "Companion " << serverEntity
                                  << " destroyed" << std::endl;
                    }
                    break;
                }
...
void ClientGameHandler::handlePlayerPacket(const DecodedMessage &msg) {
    ...
    if (it == playerEntities.end()) {
        ...
        _reg.addComponent<SpriteSheets>(localEntity, std::string(""),
                                        selectedSkin, sprW, sprH, 1, 3, 0, 0.f,
                                        0.f, true, false);
        _reg.addComponent<Stats>(localEntity, pStats.health, pStats.max_health,
                                 pStats.attack_speed, 0.f, pStats.attack_damage,
                                 1, pStats.speed);
...
void ClientGameHandler::handleUpgradeOptions(const DecodedMessage &msg) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        std::string id(msg.data.begin() + offset,
                       msg.data.begin() + offset + len);
        ids.push_back(id);
...
void ClientGameHandler::showUpgradeMenu(const std::vector<std::string> &ids) {
    ...
    for (size_t i = 0; i < ids.size(); ++i) {
        ...
        // Find data
        auto it =
            std::find_if(allUpgrades.begin(), allUpgrades.end(),
        ...
        if (it != allUpgrades.end()) {
            ...
            _reg.addComponent<Sprite>(
                btn, (std::string) "textures/upgrades/border.png",
            ...
            _reg.addComponent<Label>(
                nameLbl, data.name,
                (std::string) "font/josefin-sans/JosefinSans-Regular.ttf",
                (std::string) "default_font_medium", Color(255, 255, 255), 21,
                true);
            ...
            for (const auto &line : lines) {
                ...
                _reg.addComponent<Label>(
                    lineLbl, line,
                    (std::string) "font/josefin-sans/JosefinSans-Regular.ttf",
                    (std::string) "default_font_tiny", Color(200, 200, 200), 21,
                    true);
...
void ClientGameHandler::selectUpgrade(int index) {
    ...
    MessageFactory &factory = MessageFactory::getInstance();
    PreparedMessage msg = factory.createMessage(
        OpCode::UPGRADE_SELECT, factory.encodeMessageUpgradeSelect(index));
    ...
    // Note: Protocol might not have fully mapped TCP logic on client for
    // sending? _network.sendTcp(msg); // Let's try this. Actually
    // ClientGameHandler.cpp uses Does NetworkManager have sendTcp?

    // Checking NetworkManager.hpp... (I recall reading it has sendUdp)
    // If I can't check, I'll use UDP for now as START/CONNECT use UDP/TCP
    // mixed. Safe bet: UDP with ACK? No ACK system here. But  sets
    // UPGRADE_SELECT priority to HIGH. NetworkManager probably sends HIGH via
    // UDP.
...
void ClientGameHandler::handleUpdateWeapon(const DecodedMessage &msg) {
    ...
    // Damage (4)
    int damage = (static_cast<int>(msg.data[4]) << 24) |
                 (static_cast<int>(msg.data[5]) << 16) |
                 (static_cast<int>(msg.data[6]) << 8) |
                 static_cast<int>(msg.data[7]);

    // NbBullets (4)
    int nbBullets = (static_cast<int>(msg.data[8]) << 24) |
                    (static_cast<int>(msg.data[9]) << 16) |
                    (static_cast<int>(msg.data[10]) << 8) |
                    static_cast<int>(msg.data[11]);
    ...
    if (itPlayer != playerEntities.end()) {
        ...
        if (_reg.hasComponent<Weapon>(localEntity)) {
            ...
            w.fireRate = fireRate;
            std::cout << "Updated Weapon for player " << serverEntity
                      << " (local " << localEntity << "): FR=" << fireRate
                      << " DMG=" << damage << std::endl;
        } else {
            _reg.addComponent<Weapon>(localEntity, damage, nbBullets, fireRate);
            std::cout << "Added Weapon for player " << serverEntity
                      << " (local " << localEntity << ")" << std::endl;
        }
    }
}
