...
NetworkManager::NetworkManager() : _tcpClient(_queue), _udpClient(_queue) {}

NetworkManager::~NetworkManager() { disconnect(); }

int NetworkManager::connect(const std::string &serverIp, int tcpPort,
                            int udpPort) {
    ...
    std::cout << "Connected to server " << serverIp << " (TCP:" << tcpPort
...
void NetworkManager::start() {
    ...
    std::cout << "NetworkManager started - TCP and UDP clients running"
              << std::endl;
}
...
std::optional<DecodedMessage> NetworkManager::popMessage(Priority priority) {
    auto msg = _queue.pop(priority);
    if (msg.has_value()) {
        std::cout << "[NetworkManager] Popped message (Priority: "
                  << (int)priority << ", OpCode: " << (int)msg->opCode << ")"
                  << std::endl;
    }
    return msg;
}

bool NetworkManager::hasMessages() const { return !_queue.isEmpty(); }
