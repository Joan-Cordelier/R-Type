...
public:
    ...
    void setEnemyTypes(const std::map<std::string, EnemyTypeData> &types) {
        _enemyTypes = types;
    }
    void setBosses(const std::map<std::string, BossConfig> &bosses) {
        _bosses = bosses;
    }
    ...
    std::vector<std::pair<Entity, Entity>> &getNewProjectileEntitiesWithParent() {
        return newProjectileEntitiesWithParent;
    }
    std::vector<Entity> &getNewEnemyEntities() {
        return newEnemyEntities;
    }
    void startSpawning() {
        canSpawn = true;
    }
    std::vector<Entity> &getProjectileColliding() {
        return projectileColliding;
    }
    std::vector<Entity> &getDeadEnemyEntities() {
        return deadEnemyEntities;
    }
    ...
    void acknowledgeWaveFinished() {
        _waveFinished = false;
    }

    void setWavePaused(bool paused) {
        _isWavePaused = paused;
    }
    ...
    void setOnGameVictory(std::function<void()> callback) {
...
private:
    ...
    std::function<void()> _onGameVictory;
    bool _gameComplete = false;
    int _endlessMultiplier = 1; // For endless mode difficulty scaling
};