...
public:
  ...
  MessageData encodeMessageEnemy(Entity entity, float x, float y,
  ...
  MessageData encodeMessageProjectile(Entity projectileEntity,
                                      Entity parentEntity,
                                      const std::string &ownerType, float x,
                                      float y, float scale) const;
  MessageData encodeMessageUpdateWeapon(Entity entity, int damage,
                                        int nbBullets, float fireRate) const;
  MessageData encodeMessageMoveInput(Entity entity, float vx, float vy) const;
  MessageData
  encodeMessageUpgradeOptions(const std::vector<std::string> &upgradeIds) const;
  MessageData encodeMessageUpgradeSelect(uint8_t index) const;
  MessageData encodeMessageCompanion(Entity entity, float x, float y,
                                     uint8_t type) const;
  MessageData encodeMessageUpdateStats(Entity entity, int hp, int maxHp,
                                       int speed) const;

  MessageData encodeMessageRoomList(
