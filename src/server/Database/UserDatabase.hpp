...
struct User {
    ...
    // Endless mode high scores per difficulty (0=Easy, 1=Normal, 2=Hard, 3=Impossible)
    std::map<uint8_t, uint32_t> endlessHighScores;
};
...
public:
    ...
    getTopScores(size_t limit = 10); // Returns username, highScore pairs

    // Endless mode leaderboard (registered users only)
    void updateEndlessScore(uint32_t userId, uint8_t difficulty, uint32_t score);
    std::vector<std::pair<std::string, uint32_t>> getEndlessTopScores(uint8_t difficulty,
                                                                      size_t limit = 10);
