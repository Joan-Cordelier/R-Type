...
GameHandler::GameHandler(
    ...
    : _running(running), _session(session),
      _messageHandler(session, *inputQueue, running), _inputQueue(inputQueue) {
  // Load configuration
  char cwd[1024];
  ...
  if (!_config.loadFromFile(configPath)) {
    LOG_WARN("Failed to load config from " + configPath + ", trying ../" +
             configPath);
    if (!_config.loadFromFile("../" + configPath)) {
  ...
  LOG_INFO("Loaded Player Hitbox: W=" + std::to_string(hb.width) + " H=" +
           std::to_string(hb.height) + " OffX=" + std::to_string(hb.offset_x) +
  ...
  LOG_INFO(
      "Enemy projectile offsets: X=" + std::to_string(projConfig.offset_x) +
  ...
  enemySystem.setStatsUpdateCallback([this](Entity playerEntity) {
    if (reg.hasComponent<Stats>(playerEntity)) {
      ...
      MessageData statsPayload =
          MessageFactory::getInstance().encodeMessageUpdateStats(
              playerEntity, stats.hp, stats.maxHp, stats.movement_speed);
      PreparedMessage statsMsg = MessageFactory::getInstance().createMessage(
          OpCode::UPDATE_STATS, statsPayload);
  ...
  _messageHandler.setOnPlayerConnect(
      [this](const Player &player) { onPlayerConnect(player); });

  _messageHandler.setOnPlayerMove(
      [this](const MoveData &moveData) { onPlayerMove(moveData); });

  _messageHandler.setOnPlayerShoot(
      [this](const ShootData &shootData) { onPlayerShoot(shootData); });

  _messageHandler.setOnPlayerLink(
      [this](uint32_t playerId) { onPlayerLinked(playerId); });
  ...
  reg.addComponent<Label>(
      ScoreEntity, std::string("Score: 0"),
...
void GameHandler::sendUpdatedPositionToAllPlayers() {
  ...
  for (const auto &[playerId, entity] : playerEntities) {
    wasMoving[entity] = reg.hasComponent<Velocity>(entity) &&
                        (reg.getComponent<Velocity>(entity).vx != 0.f ||
                         reg.getComponent<Velocity>(entity).vy != 0.f);
  }
}

void GameHandler::sendNewProjectilesToAllPlayers(Entity parentEntity,
  ...
  LOG_DEBUG("Sent projectile " + std::to_string(projectileEntity) +
            ...
            " to all players at (" + std::to_string(x) + ", " +
...
void GameHandler::processMessages() {
  ...
  while (processed < maxMessagesPerFrame &&
         _messageHandler.processSingleCycle()) {
    processed++;
  }
}
...
void GameHandler::updateGame(float deltaTime) {
  ...
  if (enemySystem.isWaveFinished()) {
    ...
    if (allUpgrades.size() >= 3) {
      ...
      PreparedMessage msg =
          factory.createMessage(OpCode::UPGRADE_OPTIONS, payload);
  ...
  for (const auto &[enemy, projectile] :
       enemySystem.getNewProjectileEntitiesWithParent()) {
...
void GameHandler::onPlayerConnect(const Player &player) {
  ...
  reg.addComponent<Stats>(playerEntity, pStats.health, pStats.max_health,
  ...
  MessageData payload = factory.encodePlayerInfo(
      player.id, playerEntity, newPlayerPos.x, newPlayerPos.y);
  PreparedMessage msg = factory.createMessage(OpCode::PLAYER, payload);
  for (const auto &[existingPlayerId, existingEntity] : playerEntities) {
    _session.sendTcp(existingPlayerId, msg);
    LOG_DEBUG("Sent new player " + std::to_string(player.id) +
  ...
  if (reg.hasComponent<Stats>(playerEntity)) {
    ...
    MessageData statsPayload = factory.encodeMessageUpdateStats(
        playerEntity, s.hp, s.maxHp, s.movement_speed);
  ...
  if (reg.hasComponent<Weapon>(playerEntity)) {
    auto &w = reg.getComponent<Weapon>(playerEntity);
    MessageData weaponPayload = factory.encodeMessageUpdateWeapon(
        playerEntity, w.damage, w.nbOfBullets, w.fireRate);
    PreparedMessage weaponMsg =
        factory.createMessage(OpCode::UPDATE_WEAPON, weaponPayload);
    for (const auto &[pid, _] : playerEntities) {
  ...
  LOG_INFO("Player " + std::to_string(player.id) +
           " entity created and synced with " +
  ...
  if (it != playerEntities.end()) {
    ...
    if (_waitingForUpgrades) {
      ...
      if (playerEntities.empty() ||
          _playersSelectedUpgrade.size() >= playerEntities.size()) {
        _waitingForUpgrades = false;
        _offeredUpgrades.clear();
        enemySystem.setWavePaused(false);
...
void GameHandler::initNewEnemyEntities(
  ...
  for (const auto &enemyEntity : newEnemyEntities) {
    ...
    for (const auto &[playerId, playerEntity] : playerEntities) {
      ...
      LOG_DEBUG("Sent new enemy entity " + std::to_string(enemyEntity) + " (" +
                type + ") to player " + std::to_string(playerId));
    }
  }
}

void GameHandler::updateEnemyPosition(
...
void GameHandler::onPlayerLinked(uint32_t playerId) {
  ...
  for (Entity enemyEntity : enemySystem.getEnemyEntities()) {
    ...
    MessageData payload =
        factory.encodeMessageEnemy(enemyEntity, pos.x, pos.y, type);
  ...
  for (Entity companion : reg.viewEntitiesWith<Parent, Position>()) {
    ...
    MessageData payload =
        factory.encodeMessageCompanion(companion, pos.x, pos.y, type);
...
void GameHandler::checkPlayerCollisions() {
  ...
  for (auto projectileEntity : reg.viewEntitiesWith<Projectile, Position>()) {
    ...
    for (const auto &[playerId, playerEntity] : playerEntities) {
      // Check validity
      if (!reg.hasComponent<Position>(playerEntity) ||
          !reg.hasComponent<Stats>(playerEntity))
        continue;
      ...
      if (debugLogCounter++ % 60 == 0 &&
          playerId == playerEntities.begin()->first) {
        LOG_DEBUG("COLLISION DEBUG: PlayerPos(" + std::to_string(playerPos.x) +
                  ", " + std::to_string(playerPos.y) + ") -> Hitbox(" +
      ...
      if (collision) {
        LOG_INFO("HIT! Projectile(" + std::to_string(projLeft) + ", " +
                 ...
                 std::to_string(projH) + ") vs PlayerHitbox(" +
                 std::to_string(targetX) + ", " + std::to_string(targetY) +
                 ", " + std::to_string(pW) + ", " + std::to_string(pH) + ")");
        // Hit
        ...
        MessageData statsPayload = factory.encodeMessageUpdateStats(
            playerEntity, playerStats.hp, playerStats.maxHp,
    ...
    if (reg.hasComponent<Stats>(entity)) {
      ...
      for (const auto &effect : upgrade.effects) {
        if (effect.target == "max_health") {
          ...
          stats.attack_damage =
              (int)(stats.attack_damage * (1.0f + effect.value));
          if (weapon) {
          ...
          stats.attack_speed =
              (int)(stats.attack_speed * (1.0f + effect.value));
          if (weapon) {
          ...
          LOG_INFO("Spawning SHOTGUN companion for player " +
                   std::to_string(playerId));
          spawnCompanion(entity, WeaponType::SHOTGUN);
        } else if (effect.target == "add_weapon_missile") {
          LOG_INFO("Spawning MISSILE companion for player " +
                   std::to_string(playerId));
      ...
      if (weaponUpdated && weapon) {
        ...
        PreparedMessage msg =
            factory.createMessage(OpCode::UPDATE_WEAPON, payload);
...
void GameHandler::spawnCompanion(Entity parent, WeaponType weaponType) {
  ...
  MessageData payload = MessageFactory::getInstance().encodeMessageCompanion(
      drone, startX, startY, type);
  PreparedMessage msg =
      MessageFactory::getInstance().createMessage(OpCode::COMPANION, payload);
