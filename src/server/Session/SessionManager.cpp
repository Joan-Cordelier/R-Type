...
SessionManager::SessionManager(PrometheusExporter &monitor)
    : _tcpServer(_queue, monitor), _udpServer(_queue, monitor), _monitor(monitor) {
    _tcpServer.setOnConnect([this](int fd) { addPlayer(fd); });
...
std::unordered_map<uint32_t, Player> SessionManager::getPlayerSessions() const {
    std::lock_guard<std::mutex> lock(_playersMutex);
    return _players;
}

void SessionManager::disconnectPlayer(uint32_t playerId) {
    std::lock_guard<std::mutex> lock(_playersMutex);

    auto it = _players.find(playerId);
    if (it != _players.end()) {
        int tcpFd = it->second.tcpFd;

        // Call disconnect callback if set
        if (_onPlayerDisconnect) {
            _onPlayerDisconnect(it->second);
        }

        // Close the TCP connection
        if (tcpFd >= 0) {
            _tcpServer.disconnectClient(tcpFd);
        }

        _tcpFdToPlayerId.erase(tcpFd);
        _players.erase(it);
        _monitor.setPlayerCount(_players.size());
        LOG_INFO("Admin disconnected player " + std::to_string(playerId));
    }
}
